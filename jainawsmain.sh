...
aws ec2 create-security-group --group-name $2 --description "My security group"
aws ec2 authorize-security-group-ingress --group-name $2 --protocol tcp --port 22 --cidr 0.0.0.0/0
aws ec2 authorize-security-group-ingress --group-name $2 --protocol tcp --port 80 --cidr 0.0.0.0/0
aws ec2 authorize-security-group-ingress --group-name $2 --protocol tcp --port 443 --cidr 0.0.0.0/0 
echo "setting up ec2 instance with additional ebs and mounting"
aws ec2 run-instances --image-id ami-0217a85e28e625474 --key-name $1 --security-groups $2 --instance-type t2.micro --placement AvailabilityZone=ap-south-1a --block-device-mappings DeviceName=/dev/sdb,Ebs={VolumeSize=1} --count 1 --tag-specifications 'ResourceType=volume,Tags=[{Key=Name,Value=clivolume}]' --user-data file:///home/adminz/JAINAWS/Docker/install.txt
